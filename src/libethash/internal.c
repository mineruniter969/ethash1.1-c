	...
	ethash_light_t const light)
{
	...
	__m128i const fnv_prime = _mm_set1_epi32(FNV_PRIME_0);
	...
	{
		uint32_t parent_index = fnv_hash_0(node_index ^ i, ret->words[i % NODE_WORDS]) % num_parent_nodes;
...
#else
		{
			...
			{
				ret->words[w] = fnv_hash_0(ret->words[w], parent->words[w]);
			}
		}
#endif
	}
	...
	uint64_t const nonce)
{
	...
	{
		uint32_t const index = fnv_hash_a(s_mix->words[0] ^ i, mix->words[i % MIX_WORDS]) % num_full_pages;
		...
		for (unsigned n = 0; n != MIX_NODES; ++n)
		{
...
#if defined(_M_X64) && ENABLE_SSE
			{
				__m128i fnv_prime = _mm_set1_epi32(FNV_PRIME_B);
...
#else
			{
				for (unsigned w = 0; w != NODE_WORDS; ++w)
				{
					mix[n].words[w] = fnv_hash_b(mix[n].words[w], dag_node->words[w]);
				}
			}
#endif
		}
	...
	for (uint32_t w = 0; w != MIX_WORDS; w += 4)
	{
		...
		reduction = reduction * FNV_PRIME_E ^ mix->words[w + 1];
		reduction = reduction * FNV_PRIME_D ^ mix->words[w + 2];
		reduction = reduction * FNV_PRIME_C ^ mix->words[w + 3];
